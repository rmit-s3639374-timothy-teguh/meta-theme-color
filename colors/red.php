<?php
    ...
    try{
        ...
        $update = $db->prepare('UPDATE mtc.entries SET today = today + 1 WHERE color = "r"');
        $update->execute();

        $update = $db->prepare('UPDATE mtc.users SET today_color = "r" WHERE email = ?');
...
<body>
    <h2>You Have Chosen <?php colors('r');?></h2>

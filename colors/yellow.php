<?php
    ...
    try{
        ...
        $select = $db->prepare('SELECT today FROM mtc.entries WHERE color = "y"');
        $select->execute();
        $result = $select->fetchAll();
        $today = $result[0][0];

        $update = $db->prepare('UPDATE mtc.entries SET today = ? WHERE color = "y"');
        $update->execute(array($today + 1));

        $update = $db->prepare('UPDATE mtc.users SET today_color = "y" WHERE email = ?');
...
<body>
    <h2>You Have Chosen <?php colors('y');?></h2>
